...
export function Button({ children, variant = "primary", onClick, style, icon: Icon = ArrowRight, small, type = "button" }) {
  ...
  const variants = {
    primary: { background: C.charcoal, color: "#FFFFFF" },
  ...
  const hoverStyle = hover ? variant === "secondary" ? { background: C.charcoal, color: "#fff" } : variant === "ghost" ? { background: "rgba(255,255,255,0.12)" } : { opacity: 0.85 } : {};
  return <button type={type} onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{ ...base, ...variants[variant], ...hoverStyle, ...style }}>{children}{Icon && <Icon size={13} />}</button>;
}

export function Card({ children, style, hover = false, onClick, dark }) {
  const [isHovered, setIsHovered] = useState(false);
  return <div onClick={onClick} onMouseEnter={() => hover && setIsHovered(true)} onMouseLeave={() => hover && setIsHovered(false)} style={{ background: dark ? "#1F1F1D" : C.surface, border: `1px solid ${dark ? "rgba(255,255,255,0.12)" : C.border}`, borderRadius: RADIUS.md, padding: 22, boxShadow: isHovered ? "0 10px 24px rgba(23,23,23,0.06)" : "0 1px 2px rgba(23,23,23,0.03)", transform: isHovered ? "translateY(-2px)" : "none", transition: "all 0.22s ease", cursor: onClick ? "pointer" : "default", ...style }}>{children}</div>;
}
